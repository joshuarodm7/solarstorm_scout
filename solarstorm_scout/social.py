...
class BlueskyPoster:
    ...
    async def post_thread(self, posts: List[Dict], session: Optional[aiohttp.ClientSession] = None) -> bool:
        """
                ...
                if image_url:
                    ...
                    if img_data:
                        try:
                            ...
                            logger.warning(f"Failed to upload image for post {i+1}: {e}")
                
                # Post with reply_to for threading
                if reply_to:
                    post = self.client.send_post(text_builder, reply_to=reply_to, embed=embed)
                else:
                ...
                if image_url:
                    ...
                    if img_data:
                        try:
                            ...
                            media = self.client.media_post(tmp_path, description=alt_text)
...
class SocialMediaManager:
    ...
    def add_mastodon(self, api_base_url: str, access_token: str,
